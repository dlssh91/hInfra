...
2. **B′-2 [H-1] SECUI src_port** — 서브헤더에서 출발지 포트 컬럼 매핑 시도, 컬럼 부재가 확인되면 ISS-035 SECUI capability=False(판단보류) 강등. **어느 쪽이든 현 "영구 양호"는 제거**. [M-3]도 같이(넓은 특정범위도 지정으로 간주할지 기준 xlsx 원문 재확인).
   - **✅ 2026-07-10 Fable 설계 확정(실사 완료)**: ① SECUI 실파일 15/15 헤더영역 스캔(컬럼명만, 대외비 준수) 결과 포트 컬럼은 **'Service Port'(목적지) 단 1개 — 출발지 포트 컬럼 부재 확정** → 파싱 추가 불가, **capability `ISS-035: SECUI=False`** + `_NO_CAPABILITY_REASON_BY_FORMAT[("ISS-035", secui)]` 사유 등록(판단보류). ② **PaloAlto 동일 강등** — 파서가 `src_ports=[]`(미수집)인데 capability=True → 동일 영구양호 벡터(실데이터 0회, fail-closed 원칙) → False+사유. **ID70만 True 유지**(SVC SPEC `tcp <src> <dst>`에서 실수집). ③ **증거 조작 제거**: SECUI 파서 `src_ports=["any"]` 하드코딩 → `[]`(미수집 의미)로 정정(krfw 동일 정리, 판정 영향 0 — 두 포맷 모두 capability=False 게이트가 담당. src_ports 소비처는 detect_source_port_usage뿐임을 확인). ④ **[M-3] 광역범위 제외 삭제**: 기준 원문(정보보호시스템 장비 시트 ISS-035) = "출발지 포트 기반의 정책이 **존재**할 경우 취약"(범위 조건 없음) → `detect_source_port_usage`의 `_SENTINEL_WIDE_RANGE` 제외 분기 제거, `_is_any_port`가 아닌 모든 src_port 지정을 위반으로(ID70 any 표기 `1-65535`는 계속 제외). 과탐 아님 — needs_review=True(정당성=사람) 계약 그대로.
3. **B′-3 [H-3] 그룹객체 확장** — 아래 B-1 설계대로. 선행조건: 객체정의 매핑 소스(원본 xlsx 내 객체 시트 존재 여부, **헤더만 열람**으로 확인). dst 주소객체 우선.
   - **✅ 2026-07-10 Fable 실사·설계 확정**: 실파일 19개 전 시트 sniff 집계(시트명 원문 미열람, 카테고리 매칭만) 결과 **객체/그룹 정의 시트 0건** — 전 시트가 정책 시트(secui/krfw). → B-1의 소스①(export 내 객체시트 자동발견)은 현 데이터셋에 부재. **분할 실행**:
     - **B′-3a (선행, 객체테이블 불필요 — 거짓양호 봉쇄 본체)**: `Policy.unresolved_src/dst/svc` 신규 필드(기본 [], `policy_from_dict` .get() 하위호환) + `resolve_policies(policies, table=None)` 해석 패스(현재는 분류만: IP/포트 파싱 가능→유지, 불가→unresolved_*로 이동) + **B-1 규칙3 가드**: detect_for_iss에서 항목별 판정 관련 필드(030/031/041/034=src+dst, 032/036=svc(dst_ports), 근거로 unresolved_svc도 032/036에 귀속)에 unresolved 보유 정책 존재 ∧ 위반 0 → `양호`→`판단보류` 강등 + rationale "미해석 객체 n건 → 양호 단정 불가". 위반≥1이면 취약 유지(+미해석 부기). 033/035/037/038~040은 무영향. 직렬화 왕복 + 배치 숫자 전후 비교(dst 미파싱 35.5% 데이터셋이므로 상당수 양호→판단보류 예상 — 설계 의도).
     - **B′-3b (후속)**: `judge_tool/fw_objects.py` ObjectTable + `--aux-objects` CLI + 그룹 치환·중첩 재귀(visited+깊이8) — 합성 픽스처로 구현/검증, 실 객체 export는 현장 요청 목록(§7) 대기. resolve_policies의 table 인자에 주입하면 unresolved가 실판정으로 복귀하는 구조(B′-3a가 인터페이스를 미리 깔아둠).
4. **B′-4 [H-4]+[M-1]+[M-2] 정밀도 보정** — IP 대시범위 파싱(`ip_range→CIDR 목록` 유틸), 광역 임계 /8→/16 상향(항목별 분리 검토, 기준 xlsx 원문 근거로 확정), `_policy_covers` 포트 비대칭 수정(lower 전포트 ∧ upper 제한 → covers=False).
   - **✅ 2026-07-10(라운드3) Fable 설계 확정**: 기준 원문(ISS-031/041) 재확인 — 둘 다 "출발지 또는 목적지가 ANY 또는 **네트워크 IP대역**으로 …허용된 정책" 표현만 있고 구체적 prefix 숫자 기준은 명시 안 됨(항목별 차이도 없음) → 임계값은 원문에서 도출 불가, **기존 Opus 실사(/12~/16 미탐)를 근거로 단일 상수 채택**.
     - **H-4+M-1 통합 유틸**: `_is_broad_cidr`/`_ips_cover`가 각각 별도로 `ipaddress.ip_network()` 직접 파싱 → 대시범위(`a-b`) ValueError로 조용히 "비광역"/"미포함" 처리(미탐, 거짓양호 방향). 신규 `_parse_ip_range(ip_str) -> Optional[Tuple[int,int]]`(정수 시작·끝) 헬퍼로 **CIDR/단일IP/대시범위를 동일 표현으로 통합** — 이 표현이면 광역판정(호스트 수 임계)과 포함관계판정(구간 포함)을 **같은 정수쌍 비교**로 처리 가능해 H-4·M-1이 자연히 한 데이터경로로 묶임. 파싱 실패(형식 불명)는 기존과 동일하게 보수적 방향(비광역/미포함) 유지 — 회귀 아님, 신규 지원범위 확장만.
     - **M-1 임계**: `_BROAD_CIDR_PREFIX_LEN=8`(호스트수 임계 미사용, prefix만 비교) → **호스트수 임계 `_BROAD_RANGE_HOST_THRESHOLD=65536`(=/16 상당)**으로 교체. 대시범위도 동일 임계 적용(크기 기준이라 자연 확장). 방향: 더 넓게 탐지(미탐 축소, 안전방향) — 예상 부작용: 실데이터에 흔한 `/24`(256개) 이하는 여전히 비광역 유지(과탐 폭증 방지, /16=65536 vs /24=256 — 2단계 이상 차이로 안전마진 확보).
     - **M-2**: `_policy_covers` 포트비교가 `upper.dst_ports and lower.dst_ports` **둘 다 비어있지 않을 때만** 검사 → lower가 전포트(dst_ports=[])인데 upper가 특정포트만 허용해도 검사를 건너뛰고 "포함"으로 오판(거짓 그림자, 오탐방향). 수정: upper가 전포트가 아니면서 lower가 전포트(또는 unresolved_svc로 인한 불확정)면 **즉시 covers=False**. upper가 전포트(dst_ports=[] ∧ unresolved_svc 없음)면 무조건 포함(기존 유지). B′-3a-fix와 동형 패턴(unresolved_svc로 인한 빈 dst_ports는 "전포트"로 간주 금지 — upper/lower 양쪽에 적용).
     - **범위밖(백로그로만 기록, 이번 태스크 무관)**: 실사 중 발견 — `ADMIN_PORTS`(코드: 22/23/80/443/3389/8080/8443)가 ISS-031 기준 원문의 명시 포트목록(FTP 20/21·SSH 22·Telnet 23·MSSQL 1433/1434·Oracle 1521/1522·MySQL 3306·RDP 3389·TFTP 69·R-Service 512-514·Xmanager 177/7000/7100 등)과 상당히 다름(코드에 없는 FTP/DB포트/TFTP/R-Service/Xmanager, 원문에 없는 80/443/8080/8443). ISS-041은 원문에 구체 포트 나열 없어 `VULN_PORTS` 구성이 원 구현자의 해석 확장으로 보임. **정밀도 문제와 별개 태스크**(포트목록 완전성 재검토) — 이번 라운드 SHIP 조건에서 제외, PROGRESS.md 백로그로 인계.
