...
> 마지막 업데이트: 2026-06-18 (**상태저장. DBM 빠른 triage 진행 중 — 024~030 완료(022/026 CRITICAL 거짓양호 픽스, 025 EOL 신선도, 029 빈RESULT가드, 030 label B). 1428 passed. ★다음 착수=DBM-031(검토 제시 완료·사용자 결정 대기 — 권고=DET+빈RESULT가드, ㄱㄱ시 바로 구현).**). **"다음에 진행해줘"/"개발진행해" → 아래 ★다음 착수(DBM-031 결정/구현)부터.**
...
- **★다음 착수 = DBM-031 (검토 제시 완료·사용자 결정 대기)** (001~009·011·013·015·016·017·019·020·021·022·024·025·026·028·029·030 완료. 027 결번. 이후 032/033/034/035/036)
  **검토 방식(빠른 triage)**: 항목마다 [항목명/판단기준(xlsx)/엔진 기준(벤더 로직) **별도 표기**/이진?맥락? 라우팅 권고] 간결 제시 → 사용자 결정. det 유지면 polarity·미설정→양호만 스팟체크.

  **⏳ DBM-031 SA 계정 보안설정 — 검토 제시 완료, 결정/구현 대기 (재개 즉시 여기부터)**:
  - mssql 전용(타엔진 N/A, mssql_rds ABSENT). 판단기준: 양호=(1)sa 비활성 OR (2)sa활성+적절보안정책 / 취약=sa활성+정책미적용.
  - 엔진 로직(mssql dbm_031): `is_disabled=='0'(sa활성) AND is_policy_checked=='0'(정책미적용)` → 취약. rules/exception 미사용(로직 자체완결).
  - **스팟체크 결과(실측)**: sa비활성→양호 / sa활성+정책적용(is_policy_checked=1)→양호 / sa활성+정책미적용→취약(polarity 정확) / **빈배열(미수집)→양호 = 거짓양호 1건** / 키없음→판단보류(R3 차단).
  - **내 권고(사용자 결정 대기)**: **DET 유지 + 빈RESULT→판단보류 가드**(`_EMPTY_RESULT_HOLD`에 "DBM-031" 추가 — DBM-019/029와 동일 모드D). "적절한 보안정책"은 is_policy_checked(CHECK_POLICY)로 좁게 해석(MSSQL 표준, 결정론 타당). 사용자가 ㄱㄱ 하면 → `det_adapters/db.py` `_EMPTY_RESULT_HOLD`에 DBM-031 추가 + 회귀핀(비활성→양호/정책적용→양호/정책미적용→취약/빈배열→판단보류) + pytest + 커밋.
