...
class MariaDBCloudAnalysis:
    ...
    def dbm_013(self, result_key='DBM-013'):
        ...
        # VENDOR-EDIT: R-MA013-CLOUD — native(mariadb/analysis.py R-MA013)와 동일한
        ...
        # (§F5 거짓양호)를 차단. exception USER 제외는 유지(mariadb-config.json에서 이미
        # 빈 배열 — root 등도 검사 대상 유지). HOST='%'인 관리계정도 취약 검토 대상에
